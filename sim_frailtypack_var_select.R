...
impute_boot<-foreach(i=1:n_rep, .combine="comb", .multicombine = TRUE,  .init=list(list()), 
  ...
  foreach(b=1:n_boot_impute, .combine="comb", .multicombine = TRUE, .init=list(list()),
          .packages = c("doParallel")) %dopar%{
            ...
            # Model
            m1 <- frailtyPenal(Surv(time,event) ~ cluster(cl) + x1+x2+x3+x4+x5+x6+x7+x8+z1+z2+z3+z4+z5+z6+z7+z8+z9+z10+z11+z12+z13+z14+z15+z16+z17+z18+z19+z20, n.knots = 12, kappa = 10, data=  mice_b_dat, cross.validation=TRUE)
...
saveRDS(final_results,  paste("final_results_frailtyPenal.rds"))